...
export const getHeroes = async (): Promise<void> => {
    ...
    const formattedHeroes: HeroModel[] = Object.values(res.data).map((hero: HeroFromApi) => ({
        id: hero.id,
    ...
    fs.writeFile(`${resultDirectory}/heroes.json`, JSON.stringify(formattedHeroes), 'utf8', (err): void => {
        if (err) {
